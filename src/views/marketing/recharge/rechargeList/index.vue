    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <div class="filter-container">
          ...
          <el-button size="small" type="primary" @click="createActivity(0)">添加充值活动</el-button>
        </div>
      </div>
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="small" highlight-current-row>
        ...
        <el-table-column prop="title" label="充值活动名称" min-width="230" />
...
<script>
  import {
    activityIssueRListApi,
    activityIssueRStatusApi,
    activityRDeleApi,
  } from "@/api/marketing";
  export default {
    name: "ActivityList",
    ...
    // 删除
      handleDelete(id, idx) {
        this.$modalSure().then(() => {
          activityRDeleApi(id).then(({ message }) => {
      ...
      getList() {
        ...
        activityIssueRListApi(this.tableFrom)
          .then(res => {
      ...
      // 修改活动状态
      onchangeIsShow(row) {
        activityIssueRStatusApi(row.activity_id, row.status)
          .then((res) => {
      ...
      editActivity(data) {
        this.$router.push({
          path: `editRActivity/${data.activity_id}`,
          query: {
            title: data.title
          }
        });
      ...
      //跳转添加活动页面
      createActivity(id) {
        this.$router.push(`createRActivity/${id}`);
      },
