    ...
    <el-card class="box-card">
        <div slot="header" class="clearfix">
            <div class="container">
                ...
                <router-link :to=" { path:`${roterPre}` + '/marketing/studio2/creatStudio' } ">
        ...
        <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="small" highlight-current-row>
            ...
            <el-table-column label="小程序端" min-width="70">
                <template scope="scope">
                    <span>{{ scope.row.pro_type==1?"C端":"B端"}}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column v-if="tableForm.status_tag !== -1" key="5" label="显示状态" min-width="100">
                <template slot-scope="scope">
                    <el-switch v-model="scope.row.is_show" :active-value="1" :inactive-value="0" active-text="显示" inactive-text="隐藏" @click.native="onchangeIsShow(scope.row)" />
                </template>
            </el-table-column>
...
import importGoods from '@/components/importGoods/index'
  import {changeGroupApi} from "@/api/user";
export default {
    name: 'StudioList',
    components: {
        detailsFrom,
