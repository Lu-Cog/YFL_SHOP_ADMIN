    ...
    <el-card class="box-card">
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini">
        ...
        <el-table-column v-if="tableFrom.presell_type === '1'" label="成功/参与人数"  min-width="90" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.tattend_one.pay }} / {{ scope.row.tattend_one.all }}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="tableFrom.presell_type === '2'" :label="'第一阶段 | 成功/参与人数'" :render-header="renderheader" min-width="90" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.tattend_one.pay }} / {{ scope.row.tattend_one.all }}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="tableFrom.presell_type === '2'" :label="'第二阶段 | 成功/参与人数'" :render-header="renderheader" min-width="90" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.tattend_two.pay }} / {{ scope.row.tattend_two.all }}</span>
          </template>
        </el-table-column>
        <el-table-column label="已售商品数" min-width="90">
          <template slot-scope="scope">
            <span>{{ scope.row.seles }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.product_status === 1" type="text" size="small" class="mr10" @click="toOff(scope.row.product_id)">强制下架</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
