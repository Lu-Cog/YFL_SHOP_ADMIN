  ...
  <div class="divBox">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        ...
        <router-link :to="{path: roterPre +'/cms/addPromote'}">
      ...
      >
        ...
        <el-table-column label="文章标题" min-width="180">
          <template slot-scope="scope">
            <span>{{ scope.row.category ? ' [ ' + scope.row.category.title + ' ] ' + scope.row.title : scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="create_time"
        ...
        <el-table-column label="操作" min-width="100" fixed="right">
          <template slot-scope="scope">
            <router-link :to="{path: roterPre + '/cms/addPromote/' + scope.row.article_id}">
...
<script>
import {
  articlePromoteLstApi, articlePromoteDeleApi
} from '@/api/cms'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      articlePromoteLstApi(this.tableFrom).then(res => {
    ...
    handleDelete(id, idx) {
      this.$modalSure().then(() => {
        articlePromoteDeleApi(id).then(({ message }) => {
