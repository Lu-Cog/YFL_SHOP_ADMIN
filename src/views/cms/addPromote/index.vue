    ...
    <el-card class="box-card">
      ...
      <el-form ref="formValidate" class="form" :model="formValidate" label-width="120px" :rules="ruleValidate" @submit.native.prevent>
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="24">
            <el-form-item label="多图：" prop="slider_image">
              ...
              <div class="acea-row">
                ...
                <div v-if="formValidate.slider_image.length < 10" class="uploadCont" title="100*100px">
...
<script>
import { categoryListApi, articlePromoteAddApi, articlePromoteDetailApi,articlePromoteEditApi, articleEditApi ,articleGetTempKeysApi} from '@/api/cms'
...
export default {
  ...
  methods: {
    ...
    // 返回
    back() {
      this.$router.push({ path: `${roterPre}/cms/articlePromote` })
    },
    zh_uploadFile () {
      if (this.videoLink) {
    ...
    // 所有分类
    getList() {
      categoryListApi({pid:6}).then(res => {
    ...
    modalPicTap (tit, num, i) {
      ...
      this.$modalUpload(function(img) {
        ...
        if(tit==='1'&& !num){
          ...
          console.log(_this.formValidate.slider_image)
          img.map((item) => {
    ...
    onsubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.$route.params.id) {
            articlePromoteEditApi(this.formValidate, this.$route.params.id).then(async res => {
            ...
            articlePromoteAddApi(this.formValidate).then(async res => {
              this.$message.success(res.message)
              setTimeout(() => {
                this.$router.push({ path: `${roterPre}/cms/articlePromote` })
              }, 500)
    ...
    // 文章详情
    getDetails() {
      articlePromoteDetailApi(this.$route.params.id ? this.$route.params.id : 0).then(async res => {
        const news = res.data
