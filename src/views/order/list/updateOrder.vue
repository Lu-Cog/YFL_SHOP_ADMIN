    ...
    <el-card class="box-card">
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="small" highlight-current-row>
        ...
        <el-table-column min-width="200" label="商品名称">
          <template slot-scope="{ row }">
            <el-input class="elinput" :disabled="true" v-model="row.cart_info.product.store_name+''+row.cart_info.productAttr.sku"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="60">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="deleteGoods(scope.row.order_product_id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="foot" style="margin: 20px 0;">
        <el-row type="flex" class="row-bg" justify="end">
          ...
          <el-col :span="4">
            <div class="grid-content">商品总金额：<el-input  @input="totalSum()" class="grid-input"   v-model="orderTotal"></el-input>
            </div>
          </el-col>
          <el-col :span="4">
            <div class="grid-content">订单运费：<el-input  @input="totalSum()" class="grid-input" v-model="orderDatalist.total_postage">
              </el-input>
            </div>
          </el-col>
          <el-col :span="4">
    ...
    methods: {
      ...
      deleteGoods(order_product_id){
        ...
        this.$confirm('是否删除此商品?', '提示', {
          ...
          for (let item in this.tableData.data) {
              if(order_product_id == this.tableData.data[item].order_product_id){
                this.tableData.data.splice(item, 1);
                this.delArr.push({"order_product_id":order_product_id});
      ...
      deleteGoodsOld(id,sku) {
        ...
        this.$confirm('是否删除?', '提示', {
          ...
          deleteGoodsApi(that.updateId,id,{product_sku:sku}).then(res => {
            that.$message({
              type: 'success',
              message: '删除成功!'
            });
