    ...
    <el-card class="box-card">
      ...
      <el-form ref="formValidate" v-loading="fullscreenLoading" class="formValidate mt20" :rules="ruleValidate"
        ...
        <el-row v-show="currentTab === 0" :gutter="24">
          ...
          <el-col v-bind="grid2">
            <el-form-item label="库存预警：" prop="min_num">
              <el-input  v-model="formValidate.min_num" placeholder="请输入最小库存预警" />
            </el-form-item>
          </el-col>
          <el-col v-bind="grid2">
            <el-form-item label="库位编码：" prop="kw_code">
              <el-input  v-model="formValidate.kw_code" placeholder="请输入库位编码" />
            </el-form-item>
          </el-col>
          <el-col v-bind="grid2">
          ...
          <el-col :span="24">
            <el-form-item label="商品类型：">
              <el-radio-group v-model="formValidate.is_c">
                <el-radio :label="0">B端商品</el-radio>
                <el-radio :label="1">C端商品</el-radio>
                <el-radio :label="2">QDD端商品</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          ...
          <el-col v-if="formValidate.spec_type === 1" :span="24" class="noForm">
            ...
            <el-form-item v-if="formValidate.attr.length > 0">
              <div v-for="(item, index) in formValidate.attr" :key="index">
                ...
                <div class="rulesBox">
                  <el-tag v-for="(j, indexn) in item.detail" :key="indexn" size="medium" :disable-transitions="false"
            ...
            <el-form-item label="批量设置：" class="labeltop">
              <el-table :data="oneFormBatch" border class="tabNumWidth" size="mini">
                ...
                <el-table-column v-for="(item, iii) in attrValue" :key="iii" :label="formThead[iii].title"
                  ...
                  <template slot-scope="scope">
                    <el-input v-model="scope.row[iii]" :type="
        ...
        <el-form-item style="margin-top:30px;">
          ...
          <el-button v-show="currentTab < 2" type="primary" class="submission" size="small"
            @click="handleSubmitNest('formValidate')">下一步</el-button>
          <el-button v-show="currentTab === 2 || $route.params.id" :loading="loading" type="primary" class="submission"
    ...
    methods: {
      ...
      onChangeGroup() {
        this.checkboxGroup.includes("is_good") ?
          ...
          this.checkboxGroup.includes("is_zb") ?
          (this.formValidate.is_zb = 1) :
          (this.formValidate.is_zb = 0);
      },
      watCh(val) {
        const tmp = {};
        const tmpTab = {};
        ...
        console.log(this.ManyAttrValue, 11);
      ...
      batchAdd() {
        let flag = 0;
        console.log(this.ManyAttrValue, "add");

        for (const val of this.ManyAttrValue) {
          if (flag == 1) {
            this.$set(val, "image", this.oneFormBatch[0].image);
            this.$set(val, "price", this.oneFormBatch[0].price * this.rate);
            this.$set(val, "cost", this.oneFormBatch[0].cost * this.rate);
            this.$set(val, "ot_price", this.oneFormBatch[0].ot_price * this.rate);
            this.$set(val, "stock", this.oneFormBatch[0].stock);
            this.$set(val, "bar_code", this.oneFormBatch[0].bar_code);
            this.$set(val, "weight", this.oneFormBatch[0].weight * this.rate);
            this.$set(val, "volume", this.oneFormBatch[0].volume * this.rate);
            this.$set(val, "extension_one", this.oneFormBatch[0].extension_one);
            this.$set(val, "extension_two", this.oneFormBatch[0].extension_two);
            this.$set(val, "rate", this.rate);
        ...
        console.log(this.oneFormBatch, "add");
      },
      // 添加按钮
      addBtn() {
        this.clearAttr();
      ...
      getInfo() {
        ...
        productDetailApi(this.$route.params.id)
          .then(async res => {
            ...
            if (this.formValidate.is_zb === 1)
              this.checkboxGroup.push("is_zb");
      ...
      handleSubmit(name) {
        ...
        if (this.formValidate.spec_type === 1) {
          if(this.rate==1){
            ...
            this.ManyAttrValue[0].rate = 1
            this.ManyAttrValue[1].rate = this.rate
            this.formValidate.attrValue = this.ManyAttrValue;
          }
        } else {
          this.OneattrValue[0].rate = 1;
          ...
          if (valid) {
            ...
            this.$route.params.id ?
              ...
              .then(async res => {
                ...
                this.$router.push({
                  path: this.roterPre + "/product/examine"
                });
