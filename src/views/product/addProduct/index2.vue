...
import { shippingListApi, templateLsitApi, productCreateApi, productDetailApi,  categoryListApi, brandListApi, categoryBrandListApi, productUpdateApi, productConfigApi,productGetTempKeysApi } from '@/api/product'
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            this.$route.params.id?productUpdateApi(this.$route.params.id,this.formValidate).then(async res => {
              ...
              this.fullscreenLoading = false
              this.$message.success(res.message);
              this.$router.push({ path: this.roterPre + '/product/examine' });
              this.$refs[name].resetFields();
              this.formValidate.slider_image = []
              this.loading = false
            }).catch(res => {
