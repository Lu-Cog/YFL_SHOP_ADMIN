          ...
          <el-col :span="24">
            <el-form-item label="商品轮播图：" prop="slider_image">
              ...
              <div class="acea-row">
                ...
                >
                  <img :src="item">
                  <i class="el-icon-error btndel" @click="handleRemove(index3)" />
                </div>
          ...
          <el-col :span="24">
            <el-form-item label="商品推荐：">
              <el-checkbox-group v-model="checkboxGroup" size="small" @change="onChangeGroup">
                <el-checkbox v-for="(item, index) in recommend" :key="index3" :label="item.value">{{ item.name }}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </el-col>
          <el-col :span="24">
        ...
        <el-row v-show="currentTab === 3">
          ...
          <el-col v-if="formValidate.spec_type === 1" :span="24" class="noForm">
            ...
            <el-form-item v-if="formValidate.attr.length > 0">
              <div v-for="(item, index) in formValidate.attr" :key="index3">
                <div class="acea-row row-middle"><span class="mr5">{{ item.value }}</span><i class="el-icon-circle-close" @click="handleRemoveAttr(index3)" /></div>
                <div class="rulesBox">
                  <el-tag
                  ...
                  <el-input
                    ...
                    class="input-new-tag"
                    size="small"
                    @keyup.enter.native="createAttr(item.detail.attrsVal,index3)"
                    @blur="createAttr(item.detail.attrsVal,index3)"
                  />
...
import { shippingListApi, templateLsitApi, productCreateApi, productDetailApi, categorySelectApi, categoryListApi, categoryBrandListApi, productUpdateApi, productConfigApi,productGetTempKeysApi } from '@/api/product'
  ...
  methods: {
    ...
    watCh(val) {
      ...
      this.ManyAttrValue.forEach((val, index) => {
        ...
        if (this.attrInfo[key]) this.ManyAttrValue[index3] = this.attrInfo[key]
    ...
    // 删除表格中的属性
    delAttrTable(index) {
      this.ManyAttrValue.splice(index3, 1)
    },
    // 批量添加
    batchAdd() {
    ...
    handleRemoveAttr(index) {
      this.formValidate.attr.splice(index3, 1)
      this.manyFormValidate.splice(index3, 1)
    },
    // 删除属性
    handleClose(item, index) {
      item.splice(index3, 1)
    },
    // 添加规则名称
    createAttrName() {
