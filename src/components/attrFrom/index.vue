<template>
  <div>
    <el-form ref="formDynamic" size="small" :model="formDynamic" :rules="rules" class="attrFrom mb20" label-width="100px" @submit.native.prevent>
      ...
      <el-form-item>
        <el-button v-if="formDynamic && formDynamic.template_value.length==0 && isBtn==false" type="primary" icon="md-add" class="mt10" @click="addBtn">添加新规格</el-button>
      </el-form-item>
  ...
  methods: {
    ...
    createAttrName() {
      if (this.attrsName && this.attrsVal) {
        if(this.formDynamic.template_value.length>=1){
          this.$message.warning('不支持多规格！');
          return
        }
        const data = {
