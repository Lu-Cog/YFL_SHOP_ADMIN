                ...
                <el-tree ref="tree" :data="treeData2" :filter-node-method="filterNode" :props="defaultProps"
                  ...
                  <div slot-scope="{ node, data}" class="custom-tree-node" @click.stop="handleNodeClick(data)">
                    <div>
                      ...
                      <span v-if="data.space_property_name"
                        style="font-size: 11px;color: #3889b1">（{{ data.attachment_category_name }}）</span>
                    </div>
                    <span class="el-ic">
                      ...
                      <!--                        class="icon-space"-->
                      <!--                      />-->
                      <i v-if="data.space_id!='0' && (!data.children || data.children == 'undefined') && data.attachment_category_id"
                        class="el-icon-edit" title="修改" @click.stop="onEdit(data.attachment_category_id)" />
                      <!--                      <svg-icon-->
                      <!--                        icon-class="detail"-->
                      <!--                        title="查看该空间详情"-->
                      <!--                        class="icon-space"-->
      ...
      <el-col v-bind="grid2" class="colLeft">
        <div v-loading="loading" class="conter">
          <div class="bnt">
            ...
            <el-input v-model="tableData.attachment_name" placeholder="请输入图片名称搜索" style="width: 200px;">
            ...
            <div class="conters">
              <div v-for="(item, index) in pictrueList.list" :key="index" class="gridPic">
                ...
                <div style="display: flex;align-items: center;justify-content: space-between">
                  <p class="name" style="width: 80%">{{item.attachment_name}}</p>
                  <i class="el-icon-edit" @click="handleEdit(item.attachment_id)" />
                </div>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 编辑图片名称
      handleEdit(id) {
        this.$modalForm(picNameUpdateApi(id)).then(() => this.getFileList(''))
      },
      // 添加分类
      onAdd(id) {
        const config = {}
      ...
      changImage(item, index, row) {
        ...
        // if (this.$route.fullPath !== "/admin/config/picture") {
